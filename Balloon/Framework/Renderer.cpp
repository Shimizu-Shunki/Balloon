...
void Renderer::Render()
{
	...
	// オブジェクトを描画を行う
	for (const auto &object : m_modelRenderableObjects)
	{
...
void Renderer::UIRender()
{
	...
	// UIの描画を行う
	for (const auto& object : m_UIRenderableObjects)
	{
		// 非アクティブの場合は描画を行わない
		if (!object.first->GetIsActive() || !object.second->GetIsActive()) break;

		// 頂点バッファの更新
		UIVertexBuffer buffer = object.second->GetVertexBufferData();
		...
		// ピクセルシェーダーの設定
		m_context->PSSetShader(object.second->GetPixelShader(), nullptr, 0);

		// テクスチャの設定
		ID3D11ShaderResourceView* resorcces[] = { object.second->GetTexture() , object.second->GetRuleTexture() };
		m_context->PSSetShaderResources(0, 0, &resorcces[1]);
		m_context->PSSetShaderResources(0, 1, &resorcces[0]);
