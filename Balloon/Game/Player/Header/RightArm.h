...
class RightArm : public IComposite
{
public:
...
public:
	// コンストラクタ
	RightArm(CollisionManager* collisionManger, IComponent* parent, const int balloonIndex, const DirectX::SimpleMath::Vector3& initialPosition, const float& initialAngle);
	// デストラクタ
	~RightArm();
...
public:
	...
	// 衝突があった時
	void OnCollisionEnter(IComponent* object) override;
	// 衝突している時
	void OnCollisionStay(IComponent* object) override;
	// オブジェクトと離れたとき
	void OnCollisionExit(IComponent* object) override;

	// 衝突があった時（トリガー）
	void OnTriggerEnter(IComponent* object) override;
	// 衝突している時（トリガー）
	void OnTriggerStay(IComponent* object) override;
	// オブジェクトと離れたとき（トリガー）
	void OnTriggerExit(IComponent* object) override;
