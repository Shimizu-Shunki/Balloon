...
LeftFoot::LeftFoot(IObject* root, IObject* parent, IObject::ObjectID objectID,
...
void LeftFoot::Initialize()
{
	...
	if (m_objectID == ObjectID::PLAYER_LEFT_FOOT)
	{
		// 描画オブジェクト作成
		m_renderableObject = std::make_unique<PlayerRenderableObject>(true,
			Resources::GetInstance()->GetModelResources()->GetModel(ModelKeyID::LeftFoot));
		// 初期化処理
...
void LeftFoot::Update(const float& elapsedTime)
{
	(void)elapsedTime;
...
void LeftFoot::OnMessegeAccepted(Message::MessageData messageData)
{
	(void)messageData;
}
...
// 通知する
void LeftFoot::OnKeyPressed(KeyType type, const DirectX::Keyboard::Keys& key)
{
