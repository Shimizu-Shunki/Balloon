...
RightFoot::RightFoot(IObject* root, IObject* parent, IObject::ObjectID objectID,
...
void RightFoot::Initialize()
{
	...
	if (m_objectID == ObjectID::PLAYER_RIGHT_FOOT)
	{
		// 描画オブジェクト作成
		m_renderableObject = std::make_unique<PlayerRenderableObject>(true,
			Resources::GetInstance()->GetModelResources()->GetModel(ModelKeyID::RightFoot));
		// 初期化処理
...
void RightFoot::Update(const float& elapsedTime)
{
	(void)elapsedTime;
...
void RightFoot::OnMessegeAccepted(Message::MessageData messageData)
{
	(void)messageData;
}
...
// 通知する
void RightFoot::OnKeyPressed(KeyType type, const DirectX::Keyboard::Keys& key)
{
