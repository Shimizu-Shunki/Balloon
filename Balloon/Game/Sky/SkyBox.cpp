...
void SkyBox::Initialize()
{
	...
	Resources* resources = CommonResources::GetInstance()->GetResources();

	// スカイボックス用モデルの作成
	m_skyboxModel = DirectX::GeometricPrimitive::CreateSphere(context, 2.0f, 3, false);

	// シェーダー
	m_vertexShader = resources->GetShaderResources()->GetSkyBoxVS();
	m_pixelShader = resources->GetShaderResources()->GetSkyBoxPS();
	// スカイマップ
	m_cubemap = resources->GetTextureResources()->GetCubeMap();
	m_eveningCubeMap = resources->GetTextureResources()->GetEveningCubeMap();
