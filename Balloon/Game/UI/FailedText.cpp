...
const float FailedText::TEXTURE_HEIGHT = 237.0f;
const float FailedText::TEXTURE_WIDTH  = 754.0f;
...
void FailedText::Initialize(ObjectID objectID, const bool& active)
{
	m_objectId = objectID;
	m_isActive = active;
	...
	m_transform->SetRect({ 0.0f , TEXTURE_HEIGHT / 2.0f  , (float)width ,(TEXTURE_HEIGHT * 2.0f) / 2.0f });
	m_transform->SetColor(DirectX::SimpleMath::Vector4::One);

	commonResources->GetRenderManager()->AddSprite(m_image.get());
}

void FailedText::InitialTransform(
...
void FailedText::InitialMaterial(int width, int height)
{
...
void FailedText::OnObjectMessegeAccepted(Message::ObjectMessageID messageID)
{
	(void)messageID;
}

void FailedText::OnCollisionMessegeAccepted(Message::CollisionMessageID messageID, IObject* sender)
{
	(void)messageID;
	(void)sender;
}